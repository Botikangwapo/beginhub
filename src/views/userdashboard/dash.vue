...
import Dashskeliton from '../../skeleton/dashloader.vue';
    ...
    try {
        ...
        if (user) {
            ...
            recentQuizzes.value = (data || []).map(item => ({
                ...
                score: `${item.score}/100`, // I-adjust kung iba ang total score mo
...
<template>
    <main
        class="flex-1 2xl:p-6 p-4 space-y-6 bg-blue-50/20 text-blue-950 font-sans antialiased overflow-y-auto min-h-screen">
        <header
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm shadow-gray-300/30 p-6 space-y-4">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left text-xs font-medium">
                        <thead>
                            <tr class="text-blue-900 uppercase tracking-wider border-b border-blue-50 text-[10px]">
                        ...
                        <tbody class="divide-y divide-blue-50/50 text-blue-900">
                            <tr v-for="quiz in recentQuizzes" :key="quiz.id"
                                ...
                                <td class="py-4 pl-2 font-black text-blue-950/70 uppercase">{{ quiz.title }}</td>
                                <td class="py-4 font-mono text-gray-600 font-bold">{{ quiz.score }}</td>
                                <td class="py-4 text-blue-700/60 font-sans">{{ quiz.date }}</td>
                                <td class="py-4 text-right pr-2">
                                    <span
                                        :class="['px-2 py-0.5 rounded-md text-[10px] font-black tracking-wider border', quiz.status === 'PASSED' ? 'bg-green-50 text-green-400 border-green-100' : 'bg-red-50 text-red-400 border-red-100']">
                                        {{ quiz.status }}
                                    </span>
                                </td>
                            </tr>
